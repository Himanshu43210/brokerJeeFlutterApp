...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    brokerForTestingAuthUserSubject.add(
      BrokerForTestingAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<BrokerForTestingAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  BrokerForTestingAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = BrokerForTestingAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    brokerForTestingAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
...
BrokerForTestingAuthUser? currentUser;
